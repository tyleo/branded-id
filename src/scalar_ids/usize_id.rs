...
impl<TBrand: ?Sized> UsizeId<TBrand> {
    ...
    pub(crate) const fn usize_bound_pair(
        ...
        // Bound parameterized by UsizeId has the same layout as one over usize,
        // and the same holds for the range reinterpretations below.
        unsafe { transmute(bound_pair) }
    }

    pub(crate) const fn usize_range(range: Range<UsizeId<TBrand>>) -> Range<usize> {
        // SAFETY: see usize_bound_pair; UsizeId is #[repr(transparent)] over
        // usize.
