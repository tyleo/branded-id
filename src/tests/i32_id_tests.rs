use crate::{I32Id, IsizeId, U32Id, i32_id as id, isize_id, tests::util::BTest, u32_id, usize_id};
use std::{
    cmp::Ordering,
...
#[test]
fn from_i32_test() {
    let actual: I32Id<BTest> = I32Id::<BTest>::from_i32(1);
...
fn to_u32_id_truncates_negative_test() {
    ...
    let actual: U32Id<BTest> = id.to_u32_id();
    let expected = u32_id!(BTest; u32::MAX);
    assert_eq!(actual, expected);
}
...
fn to_usize_id_test() {
    ...
    let actual: crate::UsizeId<BTest> = id.to_usize_id();
...
fn to_usize_id_truncates_negative_test() {
    ...
    let actual: crate::UsizeId<BTest> = id.to_usize_id();
    let expected = usize_id!(BTest; usize::MAX);
    assert_eq!(actual, expected);
}
...
fn clone_test() {
    ...
    let actual: I32Id<BTest> = id.clone();
    let expected = id!(BTest; 1);
    assert_eq!(actual, expected);
}
...
#[test]
fn from_test() {
    let actual: I32Id<BTest> = From::from(1);
    let expected = id!(BTest; 1);
    assert_eq!(actual, expected);
}
...
#[test]
fn from_str_test() {
    let actual: I32Id<BTest> = <I32Id<BTest> as FromStr>::from_str("1").unwrap();
    let expected = id!(BTest; 1);
    assert_eq!(actual, expected);
}
...
fn max_test() {
    ...
    let actual: I32Id<BTest> = id_0.max(id_1);
    let expected = id!(2);
    assert_eq!(actual, expected);
}
...
fn min_test() {
    ...
    let actual: I32Id<BTest> = id_0.min(id_1);
    let expected = id!(1);
    assert_eq!(actual, expected);
}
...
fn clamp_test() {
    ...
    let actual: I32Id<BTest> = id_0.clamp(id_1, id_2);
    let expected = id!(2);
    assert_eq!(actual, expected);

    let actual: I32Id<BTest> = id_2.clamp(id_0, id_1);
    let expected = id!(2);
    assert_eq!(actual, expected);
}
