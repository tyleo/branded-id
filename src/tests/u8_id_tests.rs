use crate::{
    ...
    i16_id, i32_id, i64_id, i128_id, isize_id, tests::util::BTest, u8_id as id, u16_id, u32_id,
...
#[test]
fn from_u8_test() {
    let actual: U8Id<BTest> = U8Id::<BTest>::from_u8(1);
...
#[test]
fn to_i8_id_reinterprets_sign_test() {
    let id = id!(BTest; u8::MAX);
...
fn clone_test() {
    ...
    let actual: U8Id<BTest> = id.clone();
    let expected = id!(BTest; 1);
    assert_eq!(actual, expected);
}
...
#[test]
fn from_test() {
    let actual: U8Id<BTest> = From::from(1);
    let expected = id!(BTest; 1);
    assert_eq!(actual, expected);
}
...
#[test]
fn from_str_test() {
    let actual: U8Id<BTest> = <U8Id<BTest> as FromStr>::from_str("1").unwrap();
    let expected = id!(BTest; 1);
    assert_eq!(actual, expected);
}
...
fn max_test() {
    ...
    let actual: U8Id<BTest> = id_0.max(id_1);
    let expected = id!(2);
    assert_eq!(actual, expected);
}
...
fn min_test() {
    ...
    let actual: U8Id<BTest> = id_0.min(id_1);
    let expected = id!(1);
    assert_eq!(actual, expected);
}
...
fn clamp_test() {
    ...
    let actual: U8Id<BTest> = id_0.clamp(id_1, id_2);
    let expected = id!(2);
    assert_eq!(actual, expected);

    let actual: U8Id<BTest> = id_2.clamp(id_0, id_1);
    let expected = id!(2);
    assert_eq!(actual, expected);
}
