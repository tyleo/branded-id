use crate::{
    ...
    i16_id, i32_id, i64_id, i128_id, isize_id, tests::util::BTest, u8_id, u16_id, u32_id,
    u64_id as id, u128_id, usize_id,
};
use std::{
    cmp::Ordering,
...
#[test]
fn from_u64_test() {
    let actual: U64Id<BTest> = U64Id::<BTest>::from_u64(1);
...
#[test]
fn to_i64_id_reinterprets_sign_test() {
    let id = id!(BTest; u64::MAX);
...
fn clone_test() {
    ...
    let actual: U64Id<BTest> = id.clone();
    let expected = id!(BTest; 1);
    assert_eq!(actual, expected);
}
...
fn max_test() {
    ...
    let actual: U64Id<BTest> = id_0.max(id_1);
    let expected = id!(2);
    assert_eq!(actual, expected);
}
...
fn min_test() {
    ...
    let actual: U64Id<BTest> = id_0.min(id_1);
    let expected = id!(1);
    assert_eq!(actual, expected);
}
...
fn clamp_test() {
    ...
    let actual: U64Id<BTest> = id_0.clamp(id_1, id_2);
    let expected = id!(2);
    assert_eq!(actual, expected);

    let actual: U64Id<BTest> = id_2.clamp(id_0, id_1);
    let expected = id!(2);
    assert_eq!(actual, expected);
}
