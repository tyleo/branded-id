use crate::{
    ...
    i16_id, i32_id, i64_id, i128_id as id, isize_id, tests::util::BTest, u8_id, u16_id, u32_id,
...
#[test]
fn from_i128_test() {
    let actual: I128Id<BTest> = I128Id::<BTest>::from_i128(1);
...
#[test]
fn to_u128_id_reinterprets_sign_test() {
    let id = id!(BTest; -1);

    let actual: U128Id<BTest> = id.to_u128_id();
    let expected = u128_id!(BTest; u128::MAX);
    assert_eq!(actual, expected);
}
...
fn clone_test() {
    ...
    let actual: I128Id<BTest> = id.clone();
    let expected = id!(BTest; 1);
    assert_eq!(actual, expected);
}
...
#[test]
fn from_test() {
    let actual: I128Id<BTest> = From::from(1);
    let expected = id!(BTest; 1);
    assert_eq!(actual, expected);
}
...
#[test]
fn from_str_test() {
    let actual: I128Id<BTest> = <I128Id<BTest> as FromStr>::from_str("1").unwrap();
    let expected = id!(BTest; 1);
    assert_eq!(actual, expected);
}
...
fn max_test() {
    ...
    let actual: I128Id<BTest> = id_0.max(id_1);
    let expected = id!(2);
    assert_eq!(actual, expected);
}
...
fn min_test() {
    ...
    let actual: I128Id<BTest> = id_0.min(id_1);
    let expected = id!(1);
    assert_eq!(actual, expected);
}
...
fn clamp_test() {
    ...
    let actual: I128Id<BTest> = id_0.clamp(id_1, id_2);
    let expected = id!(2);
    assert_eq!(actual, expected);

    let actual: I128Id<BTest> = id_2.clamp(id_0, id_1);
    let expected = id!(2);
    assert_eq!(actual, expected);
}
